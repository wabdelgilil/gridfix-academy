...
import { mockCamaQuestions, mockCamaVolumes } from '../../../../data/quiz/cama/mock-cama';
...
export default function CamaMockExam() {
    ...
    return (
      <div className="card mt-6 overflow-hidden">
        <div className="border-b border-slate-200 bg-brand-50 px-5 py-3 text-sm font-semibold text-brand-800">
          🏆 محاكي امتحان CAMA — سيناريوهات بمجلدات GFMAM الخمسة
        </div>
        <div className="space-y-4 p-5">
          <p className="text-sm leading-relaxed text-ink-600">
            الامتحان الحقيقي: 110 سيناريوهات في 150 دقيقة (نحو 80 ثانية للسؤال). هذا المحاكي يقدّم مجموعة الأسئلة المتاحة
            ({mockCamaQuestions.length} سؤالاً) بنفس ضغط الوقت والتوزيع على مجلدات GFMAM الخمسة. النجاح من 70%.
          </p>
          <div className="grid gap-1.5 text-xs">
            {[1, 2, 3, 4, 5].map((p) => (
              <div key={p} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-1.5">
                <span className="font-semibold text-ink-700">المجلد {p}: {mockCamaVolumes[p]}</span>
                <span dir="ltr" className="font-mono text-ink-500">{mockCamaQuestions.filter((q) => q.volume === p).length} سؤال</span>
              </div>
            ))}
          </div>
          <div className="flex flex-wrap gap-2">
            {[10, 20, mockCamaQuestions.length].map((n) => (
              <button key={n} type="button" onClick={() => start(n)} className="cursor-pointer rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-700">
                {n === mockCamaQuestions.length ? `المحاكاة الكاملة (${n})` : `${n} سؤالاً`}
              </button>
            ))}
          </div>
        </div>
  ...
  if (stage === 'done') {
    return (
      <div className="card mt-6 overflow-hidden">
        <div className="border-b border-slate-200 bg-brand-50 px-5 py-3 text-sm font-semibold text-brand-800">
          📋 تقرير المحاكاة — التشخيص بالمجلدات
        </div>
        <div className="space-y-4 p-5">
          <div className={`rounded-xl p-4 text-center ${pass ? 'bg-teal-50 text-teal-800' : 'bg-rose-50 text-rose-800'}`}>
            ...
            <p className="mt-1 text-sm">{pass ? 'ممتاز — وتيرة النجاح في متناولك' : `راجع أضعف مجلد (${weakest ? mockCamaVolumes[weakest.p] : ''}) ثم أعد المحاكاة`}</p>
          </div>

          <div className="grid gap-1.5 sm:grid-cols-2">
            {byVolume.map((s) => (
              <div key={s.p} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
                <span className="font-semibold text-ink-700">المجلد {s.p}: {mockCamaVolumes[s.p]}</span>
  ...
  return (
    <div className="card mt-6 overflow-hidden">
      <div className="border-b border-slate-200 bg-brand-50 px-5 py-3 text-sm font-semibold text-brand-800">
        🏆 محاكي CAMA — سؤال {idx + 1} من {count}
      </div>
      ...
      <div className="space-y-4 p-5">
        ...
        <div>
          <p className="text-xs font-semibold text-brand-600">المجلد {q.volume}: {mockCamaVolumes[q.volume]} • {q.difficulty}</p>
