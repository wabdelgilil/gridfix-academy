...
export default function ResumeLearning({ course }: Props) {
  const [last, setLast] = useState<{ lessonId: string; title: string; stationTitle: string; at: string } | null>(null);
  ...
  const timeAgo = getTimeAgo(last.at);
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate font-bold text-ink-900">{last.title}</p>
