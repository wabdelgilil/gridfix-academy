...
import WeibullCurves from '../../../components/ui/figures/WeibullCurves.astro';
...
<WeibullCurves
  title="ثلاث مناطق سلوك معدل الفشل وفق معامل الشكل بيتا (β)"
  hazardLabel="معدل الفشل"
  timeLabel="الزمن"
  curves={[
    { key: 'infant', label: 'وفيات الرضع', note: 'معدل ينحدر مع الزمن — أسباب تركيب وجودة قطع. الصيانة بالزمن ضارة هنا.' },
    { key: 'random', label: 'فشل عشوائي', note: 'معدل ثابت مستقل عن العمر (توزيع أسي) — قياس مشروط أو RTF لغير الحرجة.' },
    { key: 'wearout', label: 'تآكل مرتبط بالعمر', note: 'معدل يرتفع مع العمر (يقارب الطبيعي عند β ≈ 3.5) — استبدال مجدول قبل الجرف.' },
  ]}
  caption="بيتا تحدد شكل منحنى معدل الفشل وبالتالي التكتيك الذي ينجح: انحدار (رضع) أو ثبات (عشوائي) أو ارتفاع (تآكل)."
/>
...
<WeibullCurves
  title="Three failure-rate behavior zones by the shape parameter beta (β)"
  hazardLabel="failure rate"
  timeLabel="time"
  curves={[
    { key: 'infant', label: 'Infant mortality', note: 'A declining rate over time — causes are installation and part quality. Time-based PM is harmful here.' },
    { key: 'random', label: 'Random failure', note: 'A constant rate independent of age (exponential) — condition monitoring or RTF for non-critical assets.' },
    { key: 'wearout', label: 'Age-related wear-out', note: 'A rising rate with age (approximating the normal at β ≈ 3.5) — scheduled replacement before the cliff.' },
  ]}
  caption="Beta shapes the failure-rate curve and therefore the tactic that works: declining (infant), constant (random), or rising (wear-out)."
/>
